...
export MAVEN323_HOME=`pwd`/apache-maven-3.2.3/bin
echo $MAVEN323_HOME
$MAVEN323_HOME/mvn --version

# ************************
# jdroid
# ************************

$MAVEN323_HOME/mvn clean install -P jdroid-test
...
then
	...
	$MAVEN323_HOME/mvn clean deploy -P jdroid-release -Dmaven.test.skip=true -Dgpg.skip=true --settings ./settings.xml
fi
...
$MAVEN323_HOME/mvn clean test -P jdroid-test

if [ "$INSTALL" = "true" ]
then
	$MAVEN323_HOME/mvn clean install -P jdroid-sample-android-uat -Dmaven.test.skip=true --settings ../settings.xml
fi

 ...
 * {@link com.facebook.model.GraphObject.Factory Factory} class) that provide strongly-typed property getters and
...
public interface GraphObject {
	...
	final class Factory {
		...
		private static Map<String, Object> createGraphObjectProxyForMap(JSONObject state) {
			...
			@SuppressWarnings("unchecked")
			Map<String, Object> graphObject = (Map<String, Object>)Proxy.newProxyInstance(
		...
		private final static class GraphObjectProxy extends ProxyBase<JSONObject> {
			...
			public String toString() {
				return String.format("GraphObject{graphObjectClass=%s, state=%s}", graphObjectClass.getSimpleName(),
					state);
			}
			...
			private final Object proxyMapMethods(Method method, Object[] args) {
				...
				if (methodName.equals(CLEAR_METHOD)) {
					JsonUtil.jsonObjectClear(state);
					return null;
				} else if (methodName.equals(CONTAINSKEY_METHOD)) {
					return state.has((String)args[0]);
				} else if (methodName.equals(CONTAINSVALUE_METHOD)) {
					return JsonUtil.jsonObjectContainsValue(state, args[0]);
				} else if (methodName.equals(ENTRYSET_METHOD)) {
					return JsonUtil.jsonObjectEntrySet(state);
					...
					return JsonUtil.jsonObjectKeySet(state);
					...
					JsonUtil.jsonObjectPutAll(state, map);
					return null;
					...
					return JsonUtil.jsonObjectValues(state);
				}
			...
			private final Object proxyGraphObjectMethods(Object proxy, Method method, Object[] args) {
				...
				if (methodName.equals(CAST_METHOD)) {
					...
					return Factory.createGraphObjectProxy(graphObjectClass, state);
					...
					return Factory.createGraphObjectProxyForMap(state);

 ...
 * lifecycle uses {@link SessionState SessionState} to indicate its state.
 * </p>
 * <p>
...
public class Session implements Serializable {
	...
	/**
	 * Session takes application id as a constructor parameter. If this is null, Session will attempt to load the
	 * application id from application/meta-data using this String as the key.
	 */
	...
	Session(Context context, String applicationId, TokenCachingStrategy tokenCachingStrategy, boolean loadTokenFromCache) {
		...
		if (TokenCachingStrategy.hasTokenInformation(tokenState)) {
			...
			if ((cachedExpirationDate == null) || cachedExpirationDate.before(now)) {
				...
				tokenInfo = AccessToken.createFromCache(tokenState);
	...
	public final String getAccessToken() {
		synchronized (lock) {
			return (tokenInfo == null) ? null : tokenInfo.getToken();
		}
	}
	...
	public final Date getExpirationDate() {
		synchronized (lock) {
			return (tokenInfo == null) ? null : tokenInfo.getExpires();
		}
	}
	...
	public final List<String> getPermissions() {
		synchronized (lock) {
			return (tokenInfo == null) ? null : tokenInfo.getPermissions();
		}
	}
		...
		synchronized (lock) {
			...
			switch (state) {
				...
				case OPENING:
					...
					postStateChange(oldState, state, new FacebookException("Log in attempt aborted."));
					break;
				...
				case OPENED_TOKEN_UPDATED:
					...
					postStateChange(oldState, state, null);
					break;
			}
		}
	}
	...
	public String toString() {
		return new StringBuilder().append("{Session").append(" state:").append(state).append(", token:").append(
			(tokenInfo == null) ? "null" : tokenInfo).append(", appId:").append(
			(applicationId == null) ? "null" : applicationId).append("}").toString();
	}
	...
	private Object writeReplace() {
		return new SerializationProxyV1(applicationId, state, tokenInfo, lastAttemptedTokenExtendDate, false,
				pendingRequest);
	}
	...
	/**
	 * Create a new Session, and if a token cache is available, open the Session and make it active without any user
	 * interaction.
	 * 
		...
		if (!started) {
			synchronized (lock) {
				...
				switch (state) {
					...
					default:
						state = SessionState.CLOSED_LOGIN_FAILED;
						postStateChange(oldState, state, new FacebookException("Log in attempt failed."));
				}
			}
		}
	}
	...
	private void open(OpenRequest openRequest, SessionAuthorizationType authType) {
		...
		synchronized (lock) {
			...
			switch (state) {
				case CREATED:
					state = newState = SessionState.OPENING;
					if (openRequest == null) {
				...
				case CREATED_TOKEN_LOADED:
					...
					if (pendingRequest == null) {
						state = newState = SessionState.OPENED;
					} else {
						state = newState = SessionState.OPENING;
					}
					break;
	...
	static boolean isPublishPermission(String permission) {
		return (permission != null)
				&& (permission.startsWith(PUBLISH_PERMISSION_PREFIX) || permission.startsWith(MANAGE_PERMISSION_PREFIX) || OTHER_PUBLISH_PERMISSIONS.contains(permission));
	...
	void setCurrentTokenRefreshRequest(TokenRefreshRequest request) {
		currentTokenRefreshRequest = request;
	}
	...
	public static class AuthorizationRequest implements Serializable {
		...
		private AuthorizationRequest(SessionLoginBehavior loginBehavior, int requestCode, List<String> permissions,
				String defaultAudience, boolean isLegacy, String applicationId, String validateSameFbidAsToken) {
			startActivityDelegate = new StartActivityDelegate() {
		...
		AuthorizationClient.AuthorizationRequest getAuthorizationClientRequest() {
			...
			return new AuthorizationClient.AuthorizationRequest(loginBehavior, requestCode, isLegacy, permissions,
					defaultAudience, applicationId, validateSameFbidAsToken, delegate);
		}
		
		// package private so subclasses can use it
		Object writeReplace() {
			return new AuthRequestSerializationProxyV1(loginBehavior, requestCode, permissions, defaultAudience.name(),

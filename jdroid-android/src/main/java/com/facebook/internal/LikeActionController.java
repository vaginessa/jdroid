...
public class LikeActionController {
	...
	/**
	 * Called from UiLifecycleHelper to process any pending likes that had resulted in the Like dialog being displayed
	 *
	 ...
	 */
	public static boolean handleOnActivityResult(Context context, final int requestCode, final int resultCode,
			final Intent data) {
		...
		if (Utility.isNullOrEmpty(objectIdForPendingController)) {
			SharedPreferences sharedPreferences = context.getSharedPreferences(LIKE_ACTION_CONTROLLER_STORE,
				Context.MODE_PRIVATE);
			
			objectIdForPendingController = sharedPreferences.getString(
				LIKE_ACTION_CONTROLLER_STORE_PENDING_OBJECT_ID_KEY, null);
		}
	...
	private static void invokeCallbackWithController(final CreationCallback callback,
				...
				handler.postDelayed(new Runnable() {
					...
					public void run() {
						...
						if (shouldClearDisk) {
							...
							broadcastContext.getSharedPreferences(LIKE_ACTION_CONTROLLER_STORE, Context.MODE_PRIVATE).edit().putInt(
								LIKE_ACTION_CONTROLLER_STORE_OBJECT_SUFFIX_KEY, objectSuffix).apply();
	...
	/**
	 * NOTE: This MUST be called ONLY via the SerializeToDiskWorkItem class to ensure that it happens on the right
	 * thread, at the right time.
	 */
	...
	private static LikeActionController deserializeFromJson(Context context, String controllerJsonString) {
		...
		try {
			...
			controller = new LikeActionController(context, Session.getActiveSession(),
			...
			controller.likeCountStringWithoutLike = controllerJson.optString(JSON_STRING_LIKE_COUNT_WITHOUT_LIKE_KEY,
				null);
			controller.socialSentenceWithLike = controllerJson.optString(JSON_STRING_SOCIAL_SENTENCE_WITH_LIKE_KEY,
				null);
			controller.socialSentenceWithoutLike = controllerJson.optString(
				JSON_STRING_SOCIAL_SENTENCE_WITHOUT_LIKE_KEY, null);
	...
	private static String getCacheKeyForObjectId(String objectId) {
		...
		return String.format("%s|%s|com.fb.sdk.like|%d", objectId,
			Utility.coerceValueIfNullOrEmpty(accessTokenPortion, ""), objectSuffix);
	}
	...
	public void toggleLike(Activity activity, Bundle analyticsParameters) {
		...
		boolean shouldLikeObject = !isObjectLiked;
		if (canUseOGPublish(shouldLikeObject)) {
			// Update UI state optimistically
			updateState(shouldLikeObject, likeCountStringWithLike, likeCountStringWithoutLike, socialSentenceWithLike,
	...
	private void updateState(boolean isObjectLiked, String likeCountStringWithLike, String likeCountStringWithoutLike,
			String socialSentenceWithLike, String socialSentenceWithoutLike, String unlikeToken) {
	...
	private void presentLikeDialog(Activity activity, Bundle analyticsParameters) {
		...
		if (likeDialogBuilder.canPresent()) {
			...
			if (!Utility.isNullOrEmpty(webFallbackUrl)) {
				boolean webFallbackShown = FacebookWebFallbackDialog.presentWebFallback(activity, webFallbackUrl,
					likeDialogBuilder.getApplicationId(), likeDialogBuilder.getAppCall(),
					getFacebookDialogCallback(analyticsParameters));
				if (webFallbackShown) {
					appEventsLogger.logSdkEvent(AnalyticsEvents.EVENT_LIKE_VIEW_DID_PRESENT_FALLBACK, null,
						analyticsParameters);
				}
			}
		}
	...
	private boolean onActivityResult(int requestCode, int resultCode, Intent data, UUID callId) {
		...
		// Look for results
		FacebookDialog.handleActivityResult(context, pendingCall, requestCode, data,
	...
	private FacebookDialog.Callback getFacebookDialogCallback(final Bundle analyticsParameters) {
		return new FacebookDialog.Callback() {
			...
			public void onComplete(FacebookDialog.PendingCall pendingCall, Bundle data) {
				...
				String socialSentenceWithoutWithoutLike = socialSentenceWithoutLike;
				...
				String unlikeToken = data.containsKey(LIKE_DIALOG_RESPONSE_OBJECT_IS_LIKED_KEY) ? data.getString(LIKE_DIALOG_RESPONSE_UNLIKE_TOKEN_KEY)
				...
				updateState(isObjectLiked, likeCountStringWithLike, likeCountStringWithoutLike, socialSentenceWithLike,
					socialSentenceWithoutWithoutLike, unlikeToken);
			}
	...
	private void storeObjectIdForPendingController(String objectId) {
		...
		context.getSharedPreferences(LIKE_ACTION_CONTROLLER_STORE, Context.MODE_PRIVATE).edit().putString(
			LIKE_ACTION_CONTROLLER_STORE_PENDING_OBJECT_ID_KEY, objectIdForPendingController).apply();
	}
	...
	private boolean canUseOGPublish(boolean willPerformLike) {
		...
		return !objectIsPage && (verifiedObjectId != null) && (session != null) && (session.getPermissions() != null)
		...
		fetchVerifiedObjectId(new RequestCompletionCallback() {
			...
			public void onComplete() {
				if (Utility.isNullOrEmpty(verifiedObjectId)) {
					...
					broadcastAction(context, LikeActionController.this, ACTION_LIKE_ACTION_CONTROLLER_DID_ERROR,
						errorBundle);
					return;
				}
				...
				requestBatch.addCallback(new RequestBatch.Callback() {
					...
					public void onBatchCompleted(RequestBatch batch) {
						...
						if (likeRequest.error != null) {
							...
							updateState(false, likeCountStringWithLike, likeCountStringWithoutLike,
								socialSentenceWithLike, socialSentenceWithoutLike, unlikeToken);
							...
							appEventsLogger.logSdkEvent(AnalyticsEvents.EVENT_LIKE_VIEW_DID_LIKE, null,
								analyticsParameters);
		...
		requestBatch.addCallback(new RequestBatch.Callback() {
			...
			public void onBatchCompleted(RequestBatch batch) {
				...
				if (unlikeRequest.error != null) {
					...
					updateState(true, likeCountStringWithLike, likeCountStringWithoutLike, socialSentenceWithLike,
						socialSentenceWithoutLike, unlikeToken);
	...
	private void refreshStatusAsync() {
		...
		fetchVerifiedObjectId(new RequestCompletionCallback() {
			...
			@Override
			public void onComplete() {
				final GetOGObjectLikesRequestWrapper objectLikesRequest = new GetOGObjectLikesRequestWrapper(
						verifiedObjectId);
				...
				requestBatch.addCallback(new RequestBatch.Callback() {
					...
					public void onBatchCompleted(RequestBatch batch) {
						if ((objectLikesRequest.error != null) || (engagementRequest.error != null)) {
							...
							Logger.log(LoggingBehavior.REQUESTS, TAG, "Unable to refresh like state for id: '%s'",
								objectId);
							return;
						}
						
						updateState(objectLikesRequest.objectIsLiked, engagementRequest.likeCountStringWithLike,
							engagementRequest.likeCountStringWithoutLike,
							engagementRequest.socialSentenceStringWithLike,
							engagementRequest.socialSentenceStringWithoutLike, objectLikesRequest.unlikeToken);
					}
				});
	...
	private void refreshStatusViaService() {
		...
		LikeStatusClient.CompletedListener callback = new LikeStatusClient.CompletedListener() {
			...
			public void completed(Bundle result) {
				...
				String likeCountWithLike = result.containsKey(NativeProtocol.EXTRA_LIKE_COUNT_STRING_WITH_LIKE) ? result.getString(NativeProtocol.EXTRA_LIKE_COUNT_STRING_WITH_LIKE)
						: likeCountStringWithLike;
				
				String likeCountWithoutLike = result.containsKey(NativeProtocol.EXTRA_LIKE_COUNT_STRING_WITHOUT_LIKE) ? result.getString(NativeProtocol.EXTRA_LIKE_COUNT_STRING_WITHOUT_LIKE)
						: likeCountStringWithoutLike;
				
				String socialSentenceWithLike = result.containsKey(NativeProtocol.EXTRA_SOCIAL_SENTENCE_WITH_LIKE) ? result.getString(NativeProtocol.EXTRA_SOCIAL_SENTENCE_WITH_LIKE)
						: LikeActionController.this.socialSentenceWithLike;
				
				String socialSentenceWithoutLike = result.containsKey(NativeProtocol.EXTRA_SOCIAL_SENTENCE_WITHOUT_LIKE) ? result.getString(NativeProtocol.EXTRA_SOCIAL_SENTENCE_WITHOUT_LIKE)
						: LikeActionController.this.socialSentenceWithoutLike;
				
				String unlikeToken = result.containsKey(NativeProtocol.EXTRA_UNLIKE_TOKEN) ? result.getString(NativeProtocol.EXTRA_UNLIKE_TOKEN)
						: LikeActionController.this.unlikeToken;
		...
		requestBatch.addCallback(new RequestBatch.Callback() {
			...
			public void onBatchCompleted(RequestBatch batch) {
				...
				if (Utility.isNullOrEmpty(verifiedObjectId)) {
					...
					logAppEventForError("get_verified_id", pageIdRequest.error != null ? pageIdRequest.error
	...
	private class GetOGObjectIdRequestWrapper extends AbstractRequestWrapper {
		...
		protected void processError(FacebookRequestError error) {
			...
			if (error.getErrorMessage().contains("og_object")) {
				...
				Logger.log(LoggingBehavior.REQUESTS, TAG, "Error getting the FB id for object '%s' : %s", objectId,
					error);
			}
		}
	...
	private class GetPageIdRequestWrapper extends AbstractRequestWrapper {
		...
		@Override
		protected void processError(FacebookRequestError error) {
			Logger.log(LoggingBehavior.REQUESTS, TAG, "Error getting the FB id for object '%s' : %s", objectId, error);
		}
	}
	...
	private class PublishUnlikeRequestWrapper extends AbstractRequestWrapper {
		...
		@Override
		protected void processError(FacebookRequestError error) {
			Logger.log(LoggingBehavior.REQUESTS, TAG, "Error unliking object with unlike token '%s' : %s", unlikeToken,
				error);
	...
	private class GetOGObjectLikesRequestWrapper extends AbstractRequestWrapper {
		...
		@Override
		protected void processError(FacebookRequestError error) {
			Logger.log(LoggingBehavior.REQUESTS, TAG, "Error fetching like status for object '%s' : %s", objectId,
				error);
	...
	private class GetEngagementRequestWrapper extends AbstractRequestWrapper {
		...
		GetEngagementRequestWrapper(String objectId) {
			...
			Bundle requestParams = new Bundle();
			requestParams.putString("fields", "engagement.fields(" + "count_string_with_like,"
		...
		@Override
		protected void processError(FacebookRequestError error) {
			Logger.log(LoggingBehavior.REQUESTS, TAG, "Error fetching engagement for object '%s' : %s", objectId, error);
	...
	private abstract class AbstractRequestWrapper {
		...
		protected void processError(FacebookRequestError error) {
			Logger.log(LoggingBehavior.REQUESTS, TAG, "Error running request for object '%s' : %s", objectId, error);
		}

...
/**
 * Defines an AsyncTask suitable for executing a Request in the background. May be subclassed by applications having
 * unique threading model needs.
 */
public class RequestAsyncTask extends AsyncTask<Void, Void, List<Response>> {
	...
	/**
	 * Constructor. Serialization of the requests will be done in the background, so any serialization- related errors
	 * will be returned via the Response.getException() method.
	 *
	...
	/**
	 * Constructor that allows specification of an HTTP connection to use for executing the requests. No validation is
	 * done that the contents of the connection actually reflect the serialized requests, so it is the caller's
	 * responsibility to ensure that it will correctly generate the desired responses.
	 *
	...
	public String toString() {
		return new StringBuilder().append("{RequestAsyncTask: ").append(" connection: ").append(connection).append(
			", requests: ").append(requests).append("}").toString();
	}

...
class AuthorizationClient implements Serializable {
	...
	void tryNextHandler() {
		if (currentHandler != null) {
			logAuthorizationMethodComplete(currentHandler.getNameForLogging(), EVENT_PARAM_METHOD_RESULT_SKIPPED, null,
				null, currentHandler.methodLoggingExtras);
		}
			...
			public void onBatchCompleted(RequestBatch batch) {
				try {
					...
					if ((fbids.size() == 2) && (fbids.get(0) != null) && (fbids.get(1) != null)
						...
						result = Result.createErrorResult(pendingRequest, "User logged in as different Facebook user.",
							null);
					}
					complete(result);
				} catch (Exception ex) {
	...
	class WebViewAuthHandler extends AuthHandler {
		...
		boolean tryAuthorize(final AuthorizationRequest request) {
			...
			WebDialog.Builder builder = new AuthDialogBuilder(getStartActivityDelegate().getActivityContext(),
					applicationId, parameters).setE2E(e2e).setIsRerequest(request.isRerequest()).setOnCompleteListener(
				listener);
		...
		private void saveCookieToken(String token) {
			...
			context.getSharedPreferences(WEB_VIEW_AUTH_HANDLER_STORE, Context.MODE_PRIVATE).edit().putString(
				WEB_VIEW_AUTH_HANDLER_TOKEN_KEY, token).apply();
		}
		
		private String loadCookieToken() {
			Context context = getStartActivityDelegate().getActivityContext();
			SharedPreferences sharedPreferences = context.getSharedPreferences(WEB_VIEW_AUTH_HANDLER_STORE,

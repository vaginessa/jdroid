...
public class FacebookDialog {
	...
	private static boolean handleCanPresent(Context context, Iterable<? extends DialogFeature> features) {
		return getProtocolVersionForNativeDialog(context, getMinVersionForFeatures(features)) != NativeProtocol.NO_PROTOCOL_AVAILABLE;
	}
	...
	private abstract static class Builder<CONCRETE extends Builder<?>> {
		...
		boolean handleCanPresent() {
			return getProtocolVersionForNativeDialog(activity, MIN_NATIVE_SHARE_PROTOCOL_VERSION) != NativeProtocol.NO_PROTOCOL_AVAILABLE;
		}
	 ...
	 * {@link FacebookDialog.checkCanPresentShareDialog()} indicates the capability is available.
	 */
	 ...
	 * FacebookDialog.checkCanPresentOpenGraphDialog();} indicates the capability is available.
	 */

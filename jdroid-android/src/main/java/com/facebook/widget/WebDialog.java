...
public class WebDialog extends Dialog {
	...
	public WebDialog(Context context, String action, Bundle parameters, int theme, OnCompleteListener listener) {
		...
		Uri uri = Utility.buildUri(ServerProtocol.getDialogAuthority(), ServerProtocol.DIALOG_PATH + action, parameters);
		url = uri.toString();
	...
	private Pair<Integer, Integer> getMargins() {
		...
		double scaleFactor;
		int scaledWidth = (int)(width / metrics.density);
		if (scaledWidth <= NO_BUFFER_SCREEN_WIDTH) {
			...
			scaleFactor = MIN_SCALE_FACTOR
					+ ((((double)(MAX_BUFFER_SCREEN_WIDTH - scaledWidth)) / ((double)(MAX_BUFFER_SCREEN_WIDTH - NO_BUFFER_SCREEN_WIDTH))) * (1.0 - MIN_SCALE_FACTOR));
		}

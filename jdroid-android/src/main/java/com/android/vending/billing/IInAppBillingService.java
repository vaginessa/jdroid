 ...
 * /Users/maxirosson/work/workspace/jdroid/jdroid-android/src/
 * main/java/com/android/vending/billing/IInAppBillingService.aidl
 */
package com.android.vending.billing;

/**
 * InAppBillingService is the service that provides in-app billing version 3 and beyond. This service provides the
 * following features: 1. Provides a new API to get details of in-app items published for the app including price, type,
 * title and description. 2. The purchase flow is synchronous and purchase information is available immediately after it
 * completes. 3. Purchase information of in-app purchases is maintained within the Google Play system till the purchase
 ...
 * RESULT_DEVELOPER_ERROR = 5 - invalid arguments provided to the API RESULT_ERROR = 6 - Fatal error during the API
 * action RESULT_ITEM_ALREADY_OWNED = 7 - Failure to purchase since item is already owned RESULT_ITEM_NOT_OWNED = 8 -
...
public interface IInAppBillingService extends android.os.IInterface {
	...
	public static abstract class Stub extends android.os.Binder implements
		...
		private static class Proxy implements com.android.vending.billing.IInAppBillingService {
			 ...
			 *         success, other response codes on failure as listed above. "DETAILS_LIST" with a StringArrayList
			 *         containing purchase information in JSON format similar to: '{ "productId" : "exampleSku", "type"
			 *         : "inapp", "price" : "$5.00", "title : "Example Title", "description" : "This is an example
			 *         description" }'
			 */
			@Override
			 ...
			 * @param type the type of the in-app item ("inapp" for one-time purchases and "subs" for subscription).
			 ...
			 *         value, RESULT_OK(0) if success, other response codes on failure as listed above.
			 ...
			 *            a continuationToken is returned in the response bundle. This method can be called again with
			 *            the continuation token to get the next set of owned skus.
			 * @return Bundle containing the following key-value pairs "RESPONSE_CODE" with int value, RESULT_OK(0) if
			 *         success, other response codes on failure as listed above. "INAPP_PURCHASE_ITEM_LIST" -
			 *         StringArrayList containing the list of SKUs "INAPP_PURCHASE_DATA_LIST" - StringArrayList
			 *         containing the purchase information "INAPP_DATA_SIGNATURE_LIST"- StringArrayList containing the
			 *         signatures of the purchase information "INAPP_CONTINUATION_TOKEN" - String containing a
			 *         continuation token for the next set of in-app purchases. Only set if the user has more owned skus
	 ...
	 *         other response codes on failure as listed above. "DETAILS_LIST" with a StringArrayList containing
	 *         purchase information in JSON format similar to: '{ "productId" : "exampleSku", "type" : "inapp", "price"
	 *         : "$5.00", "title : "Example Title", "description" : "This is an example description" }'
	 * @throws android.os.RemoteException
	 */
	public android.os.Bundle getSkuDetails(int apiVersion, java.lang.String packageName, java.lang.String type,
			android.os.Bundle skusBundle) throws android.os.RemoteException;
	 ...
	 *         success, other response codes on failure as listed above. "INAPP_PURCHASE_DATA" - String in JSON format
	 ...
	 * @param type the type of the in-app items being requested ("inapp" for one-time purchases and "subs" for
	 *            subscription).
	 * @param continuationToken to be set as null for the first call, if the number of owned skus are too many, a
	 *            continuationToken is returned in the response bundle. This method can be called again with the
	 *            continuation token to get the next set of owned skus.
	 * @return Bundle containing the following key-value pairs "RESPONSE_CODE" with int value, RESULT_OK(0) if success,
	 *         other response codes on failure as listed above. "INAPP_PURCHASE_ITEM_LIST" - StringArrayList containing
	 *         the list of SKUs "INAPP_PURCHASE_DATA_LIST" - StringArrayList containing the purchase information
